...
namespace DAL
{
    public class WjHisQsGxzDAL
    {
        ...
        //        }
        #region  获取肿瘤所有信息
        public static WjHisQsGxzModels GetFirst(long caseid)
        {
            string sql = "select * from wj_his_qs_gxz where CASE_ID=" + caseid;
            DataTable dt = DbSql.GetAll("cc_sys", sql);
            WjHisQsGxzModels model = new WjHisQsGxzModels();
            if (dt.Rows.Count > 0)
            {
        ...
        #region 获取高血脂数据包装
        public static IList<WjHisQsGxzModels> ListConvertToModel(DataTable dt)
        {
            // 定义集合    
            IList<WjHisQsGxzModels> ts = new List<WjHisQsGxzModels>();
            // 获得此模型的类型   
            Type type = typeof(WjHisQsGxzModels);
            string tempName = "";
            foreach (DataRow dr in dt.Rows)
            {
                WjHisQsGxzModels t = new WjHisQsGxzModels();
                // 获得此模型的公共属性      
