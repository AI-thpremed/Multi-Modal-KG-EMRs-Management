    ...
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            try
            {
                ...
                IList<long> all = CaseInfoDAL.GetAllPassXdm();

                for (int i = 0; i < all.Count; i++)
                {

                    Task<long> xx = test2(all[i]);
                ...
                using (var writer = new StreamWriter("qsmylist.csv"))
                {
                    using (var csv = new CsvWriter(writer))
                    {
                        csv.WriteHeader<QsMianyiNode>();


                        foreach (var line10 in qsmylist)
                        {
                            csv.WriteRecord(line10);
                        }
                    }
                }
        ...
        //private static void DataTableToCsv(string v, object )
        //{
        ...
        //所有的1900以下的都搞到1900默认
        //public static DateTime CheckYearToStandard(DateTime OrgDate)
        //{
        ...
        //眼底病现在通过创建csv的方式搞得节点
        public static async Task<long> test2(long id)
        {
            ...
            //await eyecheckdao.AddEye(lefteye, righteye);

            //await eyecheckdao.AddEyeLink(thiscase.ID);


            ////获得病例主节点创建出来的Id
            //var getid = await dao.GetIdByCaseId(record.CaseId);
            ...
            IList<YkbResult> ress = new List<YkbResult>();



            //前七项直接连接子节点从eye搞出来
            List<string> leftreslink = new List<string>();
            List<string> rightreslink = new List<string>();
            ...
            //8910从eye创造诊断子节点
            //List<Dictionary<string, string>> leftdiares= new List<Dictionary<string, string>>();

            Dictionary<string, string> leftdiares = new Dictionary<string, string>();
            Dictionary<string, string> rightdiares = new Dictionary<string, string>();


            //1112从eye创造实体然后连接到父节点
            Dictionary<string, string> leftdiares2 = new Dictionary<string, string>();
            Dictionary<string, string> rightdiares2 = new Dictionary<string, string>();




            //res_l_2 - 视网膜中央静脉阻塞▲res_l_8 - 黄斑水肿▲res_l_9 - IVR + O，PRP

            if (!string.IsNullOrEmpty(thiscase.TAG_ID_L))
            {
                string[] items = thiscase.TAG_ID_L.Split('▲');


                for (int i = 0; i < items.Length; i++)
                {

                    int FatherId = Convert.ToInt32(items[i].Split('-')[0].Replace("res_l_", ""));
                    ...
                    if (FatherId <= 7)
                    {
                        ...
                        //res_r_6-▲res_r_7-豹纹样眼底▲res_r_8-高度近视 -16.0D

                        //如果是这种情况结果六的细项为空，那么直接跳过了。这是个bug

                        if (!string.IsNullOrEmpty(items[i].Split('-')[1]))
                        {

                            List<string> temp = items[i].Split('-')[1].Split(',').ToList();

                            leftreslink.AddRange(temp);
                        }
                        else
                        {
                            leftreslinkfather.Add(FatherId);        
                    ...
                    if (FatherId <= 10)
                    {
                        switch (FatherId)
                        {
                            case 8:
                                ...
                                leftdiares.Add("其它", items[i].Split('-')[1]);


                                break;
                            case 9:

                                leftdiares.Add("手术方式", items[i].Split('-')[1]);
                                break;
                            case 10:

                                leftdiares.Add("用药情况", items[i].Split('-')[1]);
                                break;
                        }
                    ...
                    //            leftdiares2.Add("中心性浆液性脉络膜视网膜病", items[i].Split('-')[1]);


                    //            break;
                    //        case 12:

                    //            leftdiares2.Add("视神经萎缩", items[i].Split('-')[1]);
                    //            break;
            ...
            if (!string.IsNullOrEmpty(thiscase.TAG_ID_R))
            {
                string[] items = thiscase.TAG_ID_R.Split('▲');


                for (int i = 0; i < items.Length; i++)
                {
                    int FatherId = Convert.ToInt32(items[i].Split('-')[0].Replace("res_r_", ""));
                    ...
                    if (FatherId <= 7)
                    {
                        ...
                        if (!string.IsNullOrEmpty(items[i].Split('-')[1]))
                        {
                            ...
                            rightreslink.AddRange(temp);

                        }
                        else
                        {

                            rightreslinkfather.Add(FatherId);
                     ...
                     if (FatherId <= 10)
                    {
                        switch (FatherId)
                        {
                            case 8:
                                ...
                                //    ["其它"] = items[i].Split('-')[1]
                                //};
                                rightdiares.Add("其它", items[i].Split('-')[1]);
                            ...
                            case 9:
                                ...
                                //{
                                //    ["手术方式"] = items[i].Split('-')[1]
                                //};
                                rightdiares.Add("手术方式", items[i].Split('-')[1]);
                                break;
                            case 10:
                                //var obj3 = new Dictionary<string, string>
                                //{
                                //    ["用药情况"] = items[i].Split('-')[1]
                                //};
                                rightdiares.Add("用药情况", items[i].Split('-')[1]);
                                break;
                        }
                    ...
                    //            //    ["其它"] = items[i].Split('-')[1]
                    //            //};
                    //            rightdiares2.Add("中心性浆液性脉络膜视网膜病", items[i].Split('-')[1]);
                    ...
                    //            //    ["手术方式"] = items[i].Split('-')[1]
                    //            //};
                    //            rightdiares2.Add("视神经萎缩", items[i].Split('-')[1]);
                    //            break;
            ...
            //左右眼的前七项  只连接关系
            if (leftreslink.Count > 0)
            {


                 
                Link link2= new Link();
                link2.name = LinkStation.AddResultLinkToSon( thiscase.ID, leftreslink, "左眼");
                ...
                //await dao.AddResultLinkToSon(thiscase.ID, leftreslink, "左眼");

            }


            if (rightreslink.Count > 0)
            {



                Link link2 = new Link();
                link2.name = LinkStation.AddResultLinkToSon(thiscase.ID, rightreslink, "右眼");
                createLink.Add(link2);
            ...
            //左右眼的前七项  只连接关系
            if (leftreslinkfather.Count > 0)
            {



                Link link2 = new Link();
                link2.name = LinkStation.AddResultLinkToFatherDirect(thiscase.ID, leftreslinkfather, "左眼");
            ...
            bool creatlefttodia = false;
            bool creatrighttodia = false;


            //左右眼的8910只创造实体节点
            if (leftdiares.Count > 0)
            {
                ...
                foreach (KeyValuePair<string, string> x in leftdiares)
                {
                    EyeDiagnoseNode temp = new EyeDiagnoseNode();
                    temp.DisplayName = x.Key;
                    temp.CaseId = thiscase.ID;
                    temp.Lr = "左眼";
                    temp.Info = x.Value;

                    eyediagnose.Add(temp);
            ...
            if (rightdiares.Count > 0)
            {

                creatrighttodia = true;


                foreach (KeyValuePair<string, string> x in rightdiares)
                {
                    EyeDiagnoseNode temp = new EyeDiagnoseNode();
                    temp.DisplayName = x.Key;
                    temp.CaseId = thiscase.ID;
            ...
            //左右眼的11 12从eye创造实体然后连接到父节点
            if (leftdiares2.Count > 0)
            {
                ...
                foreach (KeyValuePair<string, string> x in leftdiares2)
                {
                    EyeDiagnoseNode temp = new EyeDiagnoseNode();
                    temp.DisplayName = x.Key;
                    temp.CaseId = thiscase.ID;
                ...
                Link link2 = new Link();
                link2.name = LinkStation.AddResultLinkToFather(thiscase.ID, leftdiares2, "左眼");
                createLink.Add(link2);
            ...
            if (rightdiares2.Count > 0)
            {
                ...
                foreach (KeyValuePair<string, string> x in rightdiares2)
                {
                    EyeDiagnoseNode temp = new EyeDiagnoseNode();
                    temp.DisplayName = x.Key;
                    temp.CaseId = thiscase.ID;
                ...
                Link link2 = new Link();
                link2.name = LinkStation.AddResultLinkToFather(thiscase.ID, rightdiares2, "右眼");
            ...
            if (creatlefttodia)
            {
                ...
                Link link2 = new Link();
                link2.name = LinkStation.AddResultLinkFromEye(thiscase.ID, "左眼");
                createLink.Add(link2);
            ...
            if (creatrighttodia)
            {
                ...
                Link link2 = new Link();
                link2.name = LinkStation.AddResultLinkFromEye(thiscase.ID, "右眼");
        ...
        //public static List<HisBltNode> bltlist = new List<HisBltNode>();
            ...
            ////IList<BioImageModels> fundus_images = BioImageDAL.GetAllImagesByName( id, "眼底照相", "0");




            ////if (fundus_images.Count> 0)
            ////{


            ////    IList<FundusNode> fundus= new List<FundusNode>();

            ////    for (int i = 0; i < fundus_images.Count; i++)
            ////    {
            ////        FundusNode temp = new FundusNode();


            ////        temp.DisplayName = "眼底图像";
            ...
            ////        temp.Path = fundus_images[i].INFO;



            ////        temp.Lr= LrTran(fundus_images[i].L_R);
            ...
            ////    await qsdao.AddFundus(fundus);

            ////    await qsdao.AddFundusLink(thiscase.ID);
            ...
            ////IList<BioImageModels> oct_images = BioImageDAL.GetAllImagesByName(id, "OCT", "0");




            ////if (oct_images.Count > 0)
            ////{



            ////    OCTNode octleft = new OCTNode();


            ////    OCTNode octright= new OCTNode();
            ////    OCTNode octnone = new OCTNode();
            ////    octnone.Path = "";
            ////    octleft.Path = "";
            ////    octright.Path = "";


            ////    for (int i = 0; i < oct_images.Count; i++)
            ////    {
            ////        if (oct_images[i].L_R == 0)
            ////        {
            ////            octnone.Path+=oct_images[i].ID+",";
            ////        }
            ////        if (oct_images[i].L_R == 1)
            ////        {
            ////            octright.Path += oct_images[i].ID + ",";
            ////        }
            ////        if (oct_images[i].L_R == 2)
            ////        {
            ////            octleft.Path += oct_images[i].ID + ",";
            ////        }                       
            ////    }

            ////    if (!string.IsNullOrEmpty(octnone.Path))
            ////    {

            ////        octnone.DisplayName = "OCT图像";
            ...
            ////        await qsdao.AddOct(octnone);

            ////    }
            ////    if (!string.IsNullOrEmpty(octright.Path))
            ////    {
            ////        octright.DisplayName = "OCT图像";

            ////        octright.Lr = "右眼";
            ////        octright.CaseId = id;

            ////        await qsdao.AddOct(octright);

            ////    }

            ////    if (!string.IsNullOrEmpty(octleft.Path))
            ////    {
            ////        octleft.DisplayName = "OCT图像";

            ////        octleft.Lr = "左眼";
            ////        octleft.CaseId = id;

            ////        await qsdao.AddOct(octleft);
            ...
            ////    await qsdao.AddOctLink(thiscase.ID);
        ...
        //原来的直接调用的方式搞得neo节点
        public static async Task<long> test(long id)
        {
            ...
            if (!string.IsNullOrEmpty(thiscase.TAG_ID_L))
            {
                ...
                for (int i = 0; i < items.Length; i++)
                {
                    ...
                    //YkbResult result = new YkbResult();


                    //result.DisplayName = "诊断结果";

                    //result.CaseId = thiscase.ID;
                    //result.Name = items[i].Split('-')[1];
                    //result.Lr = "左眼";

                    //result.FatherId = Convert.ToInt32(items[i].Split('-')[0].Replace("res_l_", "")) ;
            ...
            if (!string.IsNullOrEmpty(thiscase.TAG_ID_R))
            {
                ...
                for (int i = 0; i < items.Length; i++)
                {
                    ...
                    //result.FatherId = Convert.ToInt32(items[i].Split('-')[0].Replace("res_r_", ""));
                    //ress.Add(result);
            ...
            if (leftreslink.Count > 0)
            {
                ...
                //await dao.AddYkbResult(ress);

                //await dao.AddResultLink(thiscase.ID);
            ...
            if (rightreslink.Count > 0)
            {
                ...
                await dao.AddResultLinkToSon(thiscase.ID, rightreslink, "右眼");
            ...
            if (leftdiares.Count > 0)
            {
                ...
                await dao.AddEyeDiagnoseNode(thiscase.ID, leftdiares, "左眼");


                //await dao.AddResultLinkFromEye(thiscase.ID, "左眼");
            ...
            if (rightdiares.Count > 0)
            {
                ...
                await dao.AddEyeDiagnoseNode(thiscase.ID, rightdiares, "右眼");


                //await dao.AddResultLinkFromEye(thiscase.ID, "右眼");
            ...
            if (leftdiares2.Count > 0)
            {
                ...
                await dao.AddEyeDiagnoseNode(thiscase.ID, leftdiares2, "左眼");


                await dao.AddResultLinkToFather(thiscase.ID, leftdiares2, "左眼");
            ...
            if (rightdiares2.Count > 0)
            {
                ...
                await dao.AddEyeDiagnoseNode(thiscase.ID, rightdiares2, "右眼");


                await dao.AddResultLinkToFather(thiscase.ID, rightdiares2, "右眼");
            ...
            await dao.AddSexLink(thiscase.ID, thiscase.SEX);

            int ageid = thiscase.HOP_AGE / 10 + 1;

            await dao.AddAgeLink(thiscase.ID, ageid);
            ...
            if (!string.IsNullOrEmpty(zhusu.INFO_L))
            {
                ...
                await dao.AddZhusuNode(result);

                await dao.AddZhusuLink(thiscase.ID, targetZhusu, "左眼");

                await dao.AddToZhusuLink(thiscase.ID, "左眼");
            ...
            if (!string.IsNullOrEmpty(zhusu.INFO_R))
            {
                ...
                await dao.AddZhusuLink(thiscase.ID, targetZhusu, "右眼");

                await dao.AddToZhusuLink(thiscase.ID, "右眼");
            ...
            if (hisdia.STATUS == 1)
            {
                ...
                await qsdao.AddDia(dia);

                await qsdao.AddDiaLink(thiscase.ID);
            ...
            if (hisgxy.STATUS == 1)
            {
                ...
                await qsdao.AddGxy(gxy);

                await qsdao.AddGxyLink(thiscase.ID);
            ...
            if (hisgxz.STATUS == 1)
            {
                ...
                await qsdao.AddGxz(gxz);
                await qsdao.AddGxzLink(thiscase.ID);
            ...
            if (hisngs.STATUS == 1)
            {
                ...
                await qsdao.AddNgs(ngs);
                await qsdao.AddNgsLink(thiscase.ID);
            ...
            if (hisgxb.STATUS == 1)
            {
                ...
                await qsdao.AddGxb(gxb);

                await qsdao.AddGxbLink(thiscase.ID);
            ...
            if (hisqsmy.STATUS == 1)
            {
                ...
                await qsdao.AddQsMianyi(qsmianyi);
                await qsdao.AddQsMianyiLink(thiscase.ID);
            ...
            if (hisother.STATUS == 1)
            {
                ...
                await qsdao.AddQsOther(qsother);
                await qsdao.AddQsOtherLink(thiscase.ID);
            ...
            //if (left.CATARACT == 1 || right.CATARACT == 1)
            //{
            //    await qsdao.AddHisCataractLink(thiscase.ID);    
            //}
            ...
            //if (left.GLAUCOMA == 1 || right.GLAUCOMA == 1)
            //{
            //    await qsdao.AddHisGlaucomaLink(thiscase.ID);
            ...
            if (left.BLTSWM.Split('▲')[0] != "")
            {
                ...
                diaretin.Lr = "左眼";
                await qsdao.AddHisDiaRetin(diaretin);

                await qsdao.AddHisDiaRetinLink(thiscase.ID, diaretin.Lr);
            ...
            //if (left.BLTSWM.Split('▲')[0] != "" || right.BLTSWM.Split('▲')[0] != "")
            //{
            //    await qsdao.AddHisDiaRetinLink(thiscase.ID);
            ...
            if (left.BLTSWM.Split('▲')[2] != "" || left.BLTSWM.Split('▲')[4] != "")
            {
                ...
                //blt.FistDia = new LocalDate(Convert.ToDateTime(BasicHelper.checkDate(left.BLTSWM.Split('▲')[3])));
                //blt.FistDia =BasicHelper.checkDate(left.BLTSWM.Split('▲')[3]);
                ...
                blt.Lr = "左眼";
                await qsdao.AddHisBlt(blt);
                await qsdao.AddHisBltLink(thiscase.ID, blt.Lr);
            ...
            //if (left.BLTSWM.Split('▲')[2] != "" || left.BLTSWM.Split('▲')[4] != "" || right.BLTSWM.Split('▲')[2] != "" || right.BLTSWM.Split('▲')[4] != "")
            //{
            //    await qsdao.AddHisBltLink(thiscase.ID);
            ...
            if (left.WSS.Split('▲')[0] != "")
            {
                ...
                wss.Lr = "左眼";
                await qsdao.AddHisEyeWss(wss);
                await qsdao.AddHisEyeWssLink(thiscase.ID, wss.Lr);
            ...
            //if (left.WSS.Split('▲')[0] != "" || right.WSS.Split('▲')[0] != "")
            //{
            //    await qsdao.AddHisEyeWssLink(thiscase.ID);
            ...
            if (left.OTHER != "")
            {
                ...
                eyehisother.RmkQt = left.OTHER;
                eyehisother.Lr = "左眼";
                await qsdao.AddHisEyeOther(eyehisother);

                await qsdao.AddHisEyeOtherLink(thiscase.ID, eyehisother.Lr);
            ...
            //{
            //    await qsdao.AddHisEyeOtherLink(thiscase.ID);
            ...
            if (leftSl.STATUS == 1)
            {
                ...
                await eyecheckdao.AddSL(temp);


                await eyecheckdao.AddSlLink(thiscase.ID, temp.Lr);
            ...
            if (leftYy.STATUS == 1)
            {
                ...
                await eyecheckdao.AddYY(temp);


                await eyecheckdao.AddYyLink(thiscase.ID, temp.Lr);
            ...
            if (rightOther.STATUS == 2)
            {
                ...
                if (tar_right.Count > 0)
                    await eyecheckdao.AddYbqjLink(thiscase.ID, tar_right, "右眼");
            ...
            if (leftYd.STATUS == 2)
            {
                ...
                if (leftYd.HBCNV != 0)
                    tar_left.Add("黄斑CNV");
                if (tar_left.Count > 0)
                    await eyecheckdao.AddYdLink(thiscase.ID, tar_left, "左眼");
            ...
            if (rightYd.STATUS == 2)
            {
                ...
                if (tar_right.Count > 0)
                    await eyecheckdao.AddYdLink(thiscase.ID, tar_right, "右眼");
            ...
            if (leftOther.JZT != 0)
            {
                ...
                await eyecheckdao.AddEyeJztNode(temp);

                await eyecheckdao.AddEyeJztLink(thiscase.ID, "左眼");
            ...
            if (rightOther.JZT != 0)
            {
                ...
                await eyecheckdao.AddEyeJztNode(temp);
                await eyecheckdao.AddEyeJztLink(thiscase.ID, "右眼");
            ...
            if (leftYd.CANCER_STATUS != 0)
            {
                ...
                await eyecheckdao.AddEyeCancerNode(temp);
                await eyecheckdao.AddEyeCancerLink(thiscase.ID, "左眼");
            ...
            if (rightYd.CANCER_STATUS != 0)
            {
                ...
                await eyecheckdao.AddEyeCancerNode(temp);
                await eyecheckdao.AddEyeCancerLink(thiscase.ID, "右眼");
        ...
        //眼底病现在通过创建csv的方式搞得节点
        public static async Task<long> test3(long id)
        {
