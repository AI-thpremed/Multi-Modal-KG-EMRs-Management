...
GCN_URL = "http://192.168.2.241:12089/raw_sim_patient_retrieval_gcn"
GAT_URL = "http://192.168.2.241:19530/raw_sim_patient_retrieval"
URL = GAT_URL
...
def main(k=5):
    ...
    with open('/data/gaowh/data/eval.json', 'r', encoding='utf-8') as f:
        for line in f.readlines():
            ...
            # 请求
            res = requests.post(URL, data=json.dumps(graph))
...
def get_eval_data():
    ...
    导出验证数据
    """
    fp = open('data/eval.json', 'w', encoding='utf-8')
    id_dict = {}
    with open('data/g_sim_valid.json', 'r', encoding='utf-8') as f:
        for line in f.readlines():
            data = json.loads(line.strip())
            patient_id = data['example_a']['links'][0][0]
            id_dict[patient_id] = 1

    with open('data/clean_kg.json', 'r', encoding='utf-8') as f:
        for line in f.readlines():
            data = json.loads(line.strip())
            patient_id = data['links'][0][0]
            if patient_id in id_dict:
                fp.write(line)
    print(len(id_dict))
...
def get_train_data_new():
    """
    导出验证数据
    """
    fp = open('/data/gaowh/data/train.json', 'w', encoding='utf-8')
    id_dict = {}
    with open('/data/gaowh/data/g_sim_train.json', 'r', encoding='utf-8') as f:
        for line in f.readlines():
            data = json.loads(line.strip())
            patient_id = data['example_a']['links'][0][0]
            id_dict[patient_id] = 1
...
def get_train_data():
    ...
    导出验证数据
    """
    fp = open('data/train.json', 'w', encoding='utf-8')
    id_dict = {}
    with open('data/g_sim_train.json', 'r', encoding='utf-8') as f:
        for line in f.readlines():
            data = json.loads(line.strip())
            patient_id = data['example_a']['links'][0][0]
            id_dict[patient_id] = 1
