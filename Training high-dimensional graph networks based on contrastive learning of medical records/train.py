...
def train(model, graph_optimizer, train_loader, args, gradient_accumulate_steps=16):
    """
    训练一轮模型
    """
    model.train()
...
def evaluate(model, val_loader, args, eid):
    """
    评估模型
    """
    model.eval()
    ...
    with torch.no_grad():
        for idx, (token_ids, mask_ids, token_type_ids, node_type_ids, node_vectors, all_subgraph, all_adj_matrix,
            ...
            mse_results.append(mse_result.detach().cpu().numpy())
    mse_results = np.concatenate(mse_results, axis=0)
    print(args.model_type, 'eval mse::: ', eid,  np.mean(mse_results))


def main():
    parser = ArgumentParser()
    parser.add_argument("--train_path", type=str, default="data/g_sim_train.json",
                        help="Path of the train dataset for dist dataset. ")
    parser.add_argument("--valid_path", type=str, default="data/g_sim_valid.json",
                        help="train batch size.")
    parser.add_argument("--node_emb_path", type=str, default="data/node2vec.model",
                        help="node pretrain emb path.")
    parser.add_argument("--save_dir", type=str, default="data/ckpt/",
                        help="model save dir.")
    ...
    if args.model_type == 'gat':
        print('载入gat模型...')
        model = BertGAT(args, embed_matrix)
        ...
        return
    print('-------随机初始--------')
    evaluate(model, val_loader, args, 0)

    graph_optimizer = model.get_optimizer(args)
    for epoch_id in range(args.epochs):
        print('-------第' + str(epoch_id + 1) + '轮--------')
        train(model, graph_optimizer, train_loader, args)
        evaluate(model, val_loader, args, str(epoch_id + 1))
        torch.save(model.state_dict(), args.save_dir + 'g_sim_'+args.model_type+'_model_epoch' + str(epoch_id + 1) + '.pt')
